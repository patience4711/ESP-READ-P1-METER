...
//   if (nu - laatsteMeting >= 1000UL * (300 / inverterCount)) 
   if (nu - laatsteMeting >= 1000UL * 600) 
   {
     if(diagNose) ws.textAll( "300 secs passed" ); //
       laatsteMeting += 1000UL * 600 ; // increases each time with (300/inverterCount * miliseconds);
...
// otherwise crashes will occure.
  ...
  // ******************  reset the nework data and reboot in AP *************************
    ...
    if (actionFlag == 23) {
      ...
      console_Log("nu before = " + String(nu) );
      waitSerialAvailable(5); // recalculate time after change of settings
      nu = millis();
      console_Log("nu after = " + String(nu) );
      if(Serial.available()) console_Log("there is data available");
    }    
    ...
    if (actionFlag == 27) { //
      actionFlag = 0; //reset the actionflag
      console_Log("logChar = " + String(logChar));
    }
    ...
    if (actionFlag == 28) { //
      actionFlag = 0; //reset the actionflag
      if(!testTelegram) {
        console_Log("telegram is modified, reboot if you want to test");
        return;
      }
